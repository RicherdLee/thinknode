...
['Cache', 'Behavior', 'Controller', 'Session', 'Model', 'Db', 'Service', 'Logic'].forEach(function (item) {
    global[item] = function (super_, obj) {
        var super_o = thinkRequire(item);
        if(isEmpty(obj)){
            return Class(super_o, super_);
        }
        if(isString(super_)){
            super_o = thinkRequire(super_);
            if(isEmpty(super_o)){
                var mtname = super_.split('/');
                var gc = mtname[0] + '/' + mtname[1] + '/' + item;
                var path = THINK.APP_PATH + '/' + mtname[0] + '/' + item + '/' + mtname[1] + '.js';
                aliasImport(gc,path);
                super_o = thinkRequire(gc);
            }
        }else{
            super_o = super_;
        }
...
global.B = function (name, http, data) {
    ...
    if (name[1]) {
        ...
        var path = THINK.APP_PATH + '/' + name.join('/Behavior/') + 'Behavior.js';
        aliasImport(gc,path);
    }
    return thinkRequire(gc)(http).run(data);
};
...
global.D = function (name, layer, config) {
    ...
    if (name[1]) {
        ...
        var path = THINK.APP_PATH + '/' + name.join('/' + layer + '/') + layer + '.js';
        aliasImport(gc,path);
        name[0] = name[1];
    }
    return thinkRequire(gc)(name[0], config);
};
/**
 * 快速文件读取和写入
