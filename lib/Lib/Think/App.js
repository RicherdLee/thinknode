...
App.exec = function (http) {
    ...
    //如果开启动态加载分组配置
    //return this.loadConf(http.group).then(function () {
 ...
 * @returns {*}
 */
App.loadConf = function (group) {
    'use strict';
    var promise = getPromise(C(THINK.CONF));
    //加载分组配置
    if (!isEmpty(THINK.CONF[group])) {
        promise = promise.then(function () {
            return C(THINK.CONF[group]);
        });
    }
    return promise;
};
