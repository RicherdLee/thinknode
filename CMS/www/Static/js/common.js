...
(function () {
    ...
    if ($('.J_date').length) {
        head.use('jqueryui', function () {
            head.css('/Static/css/jquery-ui-1.10.3.custom.min.css');
            head.use('datePicker', function () {
                head.css('/Static/css/bootstrap-datetimepicker.min.css');
    ...
    if($(".J_select").length){
        head.use('jqueryui', function () {
            head.css("/Static/css/chosen.css");
            head.css("/Static/css/ace.min.css");
    ...
    //多选框
    if($(".J_dualselect").length){
        head.use('uniForm', function () {
            var db = $(".J_dualselect").find('.ds_arrow .arrow');	//get arrows of dual select
            var sel1 = $('.J_dualselect select:first-child');		//get first select element
            var sel2 = $('.J_dualselect select:last-child');			//get second select element
            sel1.css("width","160px");
            sel2.css("width","160px");
    ...
    if($("input.J_upload_preview").length){
        if(typeof($("input.J_upload_preview").attr("id")) !== undefined){
            ...
            +'<img class="profile-picture" style="cursor:hand" id="'+_id+'_preview" src="/Static/img/upload-pic.png" />'
            +'</a><br />'
            ...
            +'<button class="btn btn-mini" onclick="$(\'#'+_id+'_preview\').attr(\'src\',\'/Static/img/upload-pic.png\');$(\'#'+_id+'\').val(\'\');return false;">取消图片</button>'
            +'</div>');
            if($("#"+_id).val()){
    ...
    if ($('a.J_dialog').length) {
        head.use('artDialog', function () {
            head.css('/Static/js/artDialog/skins/default.css');
